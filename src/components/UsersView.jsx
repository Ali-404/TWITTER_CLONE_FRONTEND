...
const UserTab = ({user}) => {
  return (
    <button className=" min-w-[250px] rounded-lg md:min-w-max  text-start  hover:bg-orange-200 transition-all md:w-full bg-orange-100 shadow-md my-2  h-12  flex gap-3 items-center p-4">
      <img src="images/img2.jpg" className="object-cover shadow-md w-[40px] h-[40px] rounded-full"    />
      <div className="text-sm">
        <h1>{user.firstName ? (user.firstName + " " + user.lastName) : user.email}</h1>
        <span className="text-emerald-400">@cristiano</span>
      </div>
    </button>
  )
}
